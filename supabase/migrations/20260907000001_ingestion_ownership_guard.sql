-- Preserve global event ownership under replay and concurrent ingestion.
...
begin
  ...
  -- ON CONFLICT locks the existing row until this transaction ends.
  where workflow_runs.organization_id = excluded.organization_id
  returning id into v_run_id;

  if v_run_id is null then
    raise exception 'Event ID ownership conflict' using errcode = 'P4091';
  end if;

  -- Validate both ownership fields atomically, including legacy-only records.
  -- Do this before replacing children; any conflict rolls back the canonical write.
  insert into public.automation_runs (
    workflow_run_id, organization_id, event_id, client_id, workflow_id,
    feature_type, workflow_name, n8n_workflow_id, status,
    records_processed, records_failed, duration_ms, error_message, metadata, ran_at
  ) values (
  ...
  where automation_runs.client_id = excluded.client_id
    and automation_runs.organization_id = excluded.organization_id

import { browserJson as json, browserPreflight } from "../_shared/cors.ts";
import type { BrowserHandlerDependencies } from "../_shared/browser-handler.ts";
...
export const createAlertRouteHandler = ({ env, createClient }: BrowserHandlerDependencies) => async (req: Request): Promise<Response> => {
  const preflight = browserPreflight(req);
  ...
  try {
    ...
    const configuration = typeof integration.configuration === "object" && integration.configuration && !Array.isArray(integration.configuration) ? integration.configuration : {};
    ...
    return json({ error: "Failed to configure alert route" }, 500);
  }
};

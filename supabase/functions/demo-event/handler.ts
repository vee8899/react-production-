...
export const createDemoEventHandler = ({ env, createClient }: BrowserHandlerDependencies) => async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (clientError || !client?.organization_id) return json({ error: "Demo workspace required" }, 403);
    ...
    return json({ error: "Unable to process demo event" }, 500);
  }
};

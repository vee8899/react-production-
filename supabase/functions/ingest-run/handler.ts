...
export type IngestDependencies = {
  ...
  rpc: (url: string, key: string, args: Record<string, unknown>) => PromiseLike<RpcResult>;
};

export const createIngestHandler = (dependencies: IngestDependencies) => async (req: Request) => {
  if (req.method !== "POST") {
  ...
  const webhookSecret = dependencies.env("WEBHOOK_SECRET");
  if (!webhookSecret) {
  ...
  const rpcResult = await Promise.race([
    dependencies.rpc(supabaseUrl, serviceRoleKey, {
    ...
    new Promise<never>((_, reject) => {
      timeout = setTimeout(() => reject(new Error("Ingestion timed out")), 10_000);
    }),
  ...
  if (error) {
    if (typeof error === "object" && "code" in error && error.code === "P4091") {
      console.warn("Ingestion event ownership conflict", { event_id: parsed.data.event_id });
      return json({ error: "Event ID conflict", code: "event_id_conflict" }, 409);
    }
